...
export default function Home() {
  ...
  function performSummon(quantity: 1 | 10 = 1) {
    ...
    showAction('✦', banner === 'creature' ? `Creature Bell ×${quantity}` : 'Border Forge activated', 'magic');
    window.setTimeout(() => {
      if (banner === 'creature') {
        setGame((current) => {
  ...
  function purchaseUpgrade(upgradeId: UpgradeId) {
    ...
    showAction('↑', `${definition.name} · Level ${currentLevel + 1}`, 'spend');
  }
