...
import {
  BORDERS, BORDER_SUMMON_COST, SPECIES, SUMMON_COST, TEN_PULL_COST, UPGRADES, VARIANTS,
...
function CreatureArt({ speciesId, variant, size = 'medium', animated = false }: { speciesId: SpeciesId; variant?: AnimalInstance['variant']; size?: 'small' | 'medium' | 'large' | 'hero'; animated?: boolean }) {
  ...
  return <span className={`creature-art art-${size} ${variant ? `art-${variant}` : ''} ${animated ? 'active-creature-art' : ''}`} style={{ backgroundPosition: `${column * 50}% ${row * 50}%` }} role="img" aria-label={`${variant ? `${VARIANTS[variant].name} ` : ''}${SPECIES[speciesId].name}`} />;
}
  ...
  return <main className="game-shell">
    ...
    <div className="game-layout" id="game-content">
      <nav className="side-nav" aria-label="Game navigation">
        {([['farm','⌂','Farm'],['summon','✦','Summon'],['animals','▦','Animals'],['merge','⌁','Merge'],['upgrades','⚙','Upgrade'],['visit','♧','Visit']] as [View,string,string][]).map(([id, icon, label]) => <button key={id} className={view === id ? 'active' : ''} type="button" onClick={() => openView(id)}><span>{icon}</span>{label}</button>)}
      </nav>
      ...
      {view === 'summon' && <section className={`summon-screen wide-card ${isSummoning ? 'summoning' : ''}`} aria-labelledby="summon-title">
        ...
        <div className="summon-content"><p className="eyebrow">The Grand Gacha Hall</p><h2 id="summon-title">Choose what answers the bell</h2><p>Creature banners grow your herd. Border banners unlock permanent farm styles and gameplay boosts.</p>
          ...
          <div className="summon-actions"><button className="summon-main-button" type="button" disabled={isSummoning} onClick={() => performSummon(1)}><span>{isSummoning ? 'The magic is gathering…' : banner === 'creature' ? 'Summon ×1' : 'Forge a Farm Border'}</span><strong>● {(banner === 'creature' ? SUMMON_COST : BORDER_SUMMON_COST).toLocaleString()}</strong></button>{banner === 'creature' && <button className="ten-pull-button" type="button" disabled={isSummoning} onClick={() => performSummon(10)}><span>Grand Summon ×10</span><strong>● {TEN_PULL_COST.toLocaleString()}</strong><small>Save 500 · Rare+ guaranteed</small></button>}</div>
      ...
      {view === 'animals' && <section className="farm-card manage-card wide-card" aria-labelledby="collection-title">
        <div className="section-heading"><div><p className="eyebrow">Creature archive · {game.discoveredSpecies.length}/{Object.keys(SPECIES).length} discovered</p><h2 id="collection-title">Your animals</h2></div><span className="count-badge">{game.animals.length} owned</span></div>
        ...
        <div className="species-index">{Object.values(SPECIES).map((species) => { const discovered = game.discoveredSpecies.includes(species.id); return <article className={`${discovered ? 'discovered' : 'unknown'} rank-${species.rank.toLowerCase()}`} key={species.id}>{discovered ? <CreatureArt speciesId={species.id} size="small" /> : <span className="unknown-creature">?</span>}<div><strong>{discovered ? species.name : 'Undiscovered'}</strong><small>{species.rank} · {discovered ? `${game.animals.filter((animal) => animal.speciesId === species.id).length} owned` : `${species.summonWeight}% base rate`}</small></div></article>; })}</div>
      ...
      {view === 'merge' && <section className="farm-card manage-card wide-card"><div className="section-heading"><div><p className="eyebrow">Manual species merge</p><h2>Choose your three parents</h2></div><span className="count-badge">◇ {game.fusionDust}</span></div><div className="merge-explainer"><span>1. Pick an animal</span><b>→</b><span>2. Pick two matching copies</span><b>→</b><span>3. Confirm merge</span></div>
        <div className="merge-workbench"><section className="merge-tray" aria-label="Selected merge parents"><div className="merge-tray-heading"><div><span>Merge tray</span><strong>{selectedMergeAnimals.length}/3 selected</strong></div>{selectedMergeAnimals.length > 0 && <button type="button" onClick={() => setSelectedMergeIds([])}>Clear all</button>}</div><div className="merge-slots">{Array.from({ length: 3 }, (_, index) => { const animal = selectedMergeAnimals[index]; return <div className={animal ? 'filled' : ''} key={index}><span className="merge-slot-number">{index + 1}</span>{animal ? <><CreatureArt speciesId={animal.speciesId} variant={animal.variant} size="small"/><strong>{animalName(animal)}</strong><small>P{animal.potential} · +{animalIncomePerMinute(animal)}/min</small><button type="button" onClick={() => toggleMergeAnimal(animal)}>Remove</button></> : <><span className="merge-slot-plus">+</span><strong>Choose parent {index + 1}</strong><small>{mergeTemplate ? `Needs ${animalName(mergeTemplate)}` : 'Any stored animal'}</small></>}</div>; })}<span className="merge-arrow">→</span><div className="merge-result-preview"><span>?</span><strong>{mergeTemplate ? animalName(mergeTemplate) : 'New offspring'}</strong><small>Best parent stats · possible next variant</small></div></div><button className="confirm-merge-button" type="button" disabled={selectedMergeAnimals.length !== 3} onClick={performMerge}>{selectedMergeAnimals.length === 3 ? 'Merge these 3 animals' : `Choose ${3 - selectedMergeAnimals.length} more`}</button></section>
